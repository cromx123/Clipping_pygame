...
def LineaBresenham(x1, y1 , x2, y2, win, pixel_size, color="black"):
    ...
    if dx > dy:  # Si la línea es más horizontal que vertical
        ...
        x, y = x1, y1
        while x != x2 or y != y2:
            draw_pixel(x * pixel_size, y * pixel_size, win, pixel_size, color)
...
def LineaBresenham_borrado(x1, y1 , x2, y2, win, pixel_size, color="white"):
    ...
    if dx > dy:  # Si la línea es más horizontal que vertical
        ...
        x, y = x1, y1
        while x != x2 or y != y2:
            undraw_pixel(x * pixel_size, y * pixel_size, win, pixel_size)
            if x == x2 and y == y2:  # Chequeo de finalización
                break
            x += sx
            if p < 0:
...
def CircleBresenham(x1, y1, r, win, pixel_size):
    x = 0
    y = r
    p = 1 - r
...
def Representacion_Elipse(xc, yc, a, b):
